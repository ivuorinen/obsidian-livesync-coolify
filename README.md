...
- Base images pinned to exact versions in `Dockerfile`, with Dependabot proposing updates.
...
Root dotfiles configure formatting, linting, secret scanning, ignore rules, and Dependabot.
...
The CouchDB and Deno base images are pinned to exact versions in the `FROM` lines of `Dockerfile`. Dependabot proposes updates for both; review release notes before merging a CouchDB major-version upgrade.
